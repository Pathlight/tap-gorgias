...
def sync_stream(state, start_date, instance, config):
    ...
    # otherwise use start_date & update bookmark with it
    bookmark_exists = (
        state.get("bookmarks", {})
        .get(stream.tap_stream_id, {})
        .get(instance.replication_key)
    )
    if instance.replication_method == "INCREMENTAL" and not bookmark_exists:
        singer.write_bookmark(
            state, stream.tap_stream_id, instance.replication_key, start_date
        )
