...
class GorgiasAPI:
    ...
    def get(self, url):
        """GET data with built-in retries and exception handling"""
        ...
        for num_retries in range(self.MAX_RETRIES):
            ...
            try:
                ...
                if resp.status_code == 429 and num_retries < self.MAX_RETRIES:
                    ...
                    LOGGER.info(
                        "api query gorgias rate limit",
                        extra={"retry_after": retry_after, "subdomain": self.subdomain},
                    )
                    time.sleep(int(retry_after))
                elif resp.status_code >= 500 and num_retries < self.MAX_RETRIES:
                    LOGGER.info(
                        "api query gorgias 5xx error",
                        extra={"subdomain": self.subdomain},
                    )
                    time.sleep(10)
                else:
    ...
    def post(self, url, params):
        if not url.startswith("https://"):
            url = f"{self.base_url}/{url}"
        resp = requests.post(url, json=params, auth=(self.username, self.password))
        return resp.json()
